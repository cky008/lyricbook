...
> **0.0.7 重点改进歌词本打印。** 页面会根据浏览器实测结果调整字号，长歌词完整分页，目录在需要时连续分页并保持链接正确；只有排版安全检查通过后才能保存 PDF。
...
- 支持实测排版的 A4、A5、A4 对折小册、短句斜杠合并、目录跳转、页码安全区和动态字号。
- 对折小册可使用系统生成封面、本地图片封面或图片＋系统文字；图片只在浏览器本地处理，绝不上传。
